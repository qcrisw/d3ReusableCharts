...
function Scatterplot(data, chartWrapper, chartId, xAxisLabel, yAxisLabel) {
    ...
    var parentDiv = document.getElementById(chartWrapper.replace("#", ""));
    ...
    function responsivefy(svg) {
     ...
     // FEATURE - Redraw chart on window resize
        function resize() {
              ...
              console.log("ddd", targetWidth, svg.node().parentNode)
...
//              height = parentDiv.clientHeight - margin.top - margin.bottom;
              ...
              d3.select(parent + ' svg, ' + parent + ' svg g').attr('width', width).attr('height', height);

              xAxisG.call(xAxis);
              yAxisG.call(yAxis);
              d3.select(axisSelection+'>g.xAxisG>text').remove();
              d3.select(axisSelection+'>g.yAxisG>text').remove();
              d3.selectAll('data-points-grp').remove();
              d3.selectAll('data-points-indv').remove();
    ...
    svg.append('g')
	.attr('transform', `translate(0, ${height})`)
	.style('font-style', 'italic').call(xAxis);
    svg.append('g')
    .call(yAxis);
...
// ################# Functions ######################
    ...
    function drawScatterplot(jsonData, xScale, yScale, yScale2) {
      ...
      **/
      ...
      d3.selectAll('g.data-points-grp').remove();
      d3.selectAll('g.data-points-indv').remove();
      d3.selectAll("div"+parent+">svg>g>rect").remove();
      tooltip.classed('hidden', true);

      var dataCirclesG = svg.append('g')
        .attr("class", "data-points-grp");
      var dataCirclesI = svg.append('g')
    ...
    function d3ScatterplotMouseOver(d, xScale, yScale, yScale2){
        ...
        var left = Math.min(width, mouse[0]+margin.left+margin.right),
        top = Math.min(height, mouse[1]+margin.top+margin.right);
